export default class Particles {
  ...
  addStars (color, x, y) {
    let angle, i, k, radiusSQ, ref, vertex
            this.dirs = []
  ...
  getTexture = function (color) {
    let canvas, context, gradient, texture
  ...
  animate = function () {
    let i, j, k, l, particle, ref, ref1
    for (
      j = k = 0, ref = this.systemCount - 1;
      0 <= ref ? k <= ref : k >= ref;
      j = 0 <= ref ? ++k : --k
    ) {
      for (
        i = l = 0, ref1 = this.particleCount;
  ...
  mouseMove = function () {
    ...
    return (this.canvasMouse = this.camera.position
      .clone()
      .add(this.dir.multiplyScalar(this.distance)))
  }

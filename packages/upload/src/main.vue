<template>
  <div class="r-upload">
    ...
    <ol class="r-upload-fileList">
      <li v-for="file in fileList" :key="file.name">

        <template v-if="file.status === 'uploading'">
          <r-icon name="loading" class="r-upload-spin"></r-icon>
        </template>
        <template v-else-if="file.type.indexOf('image') === 0">
          <img class="r-upload-image" :src="file.url" width="32" height="32" alt="">
        </template>
        <template v-else>
          <div class="r-upload-defaultImage"></div>
        </template>

        <span class="r-upload-name" :class="{[file.status]: file.status}">{{file.name}}</span>
        <button class="r-upload-remove" @click="onRemoveFile(file)">x</button>
      </li>
    </ol>
  ...
  methods: {
    ...
    async onRemoveFile (file) {
      ...
      if (this.beforeRemove) {
        res = await this.beforeRemove(file, this.fileList)
      }
      if (!res) return
